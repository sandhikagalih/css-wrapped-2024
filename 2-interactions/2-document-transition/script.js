...
const basePath = '/2-interactions/2-document-transition';
...
document.addEventListener('click', (e) => {
  if (e.target.matches('a.back')) {
    ...
    // Fallback for browsers that donâ€™t have the Navigation API
    if (!window.navigation) {
      history.go(-1);
      return;
    }
...
window.addEventListener('pagereveal', async (e) => {
  // Simpler approach for browsers that donâ€™t support the Navigation API
  ...
  // User comes from different origin or did a reload
    ...
    // @TODO: manually create a â€œwelcomeâ€ viewTransition here?
  }
});
